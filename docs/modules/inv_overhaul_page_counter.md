...
`scripts/inventory_interface/inv_overhaul_page_counter.lua`

DSL unit: `maintask InvOverhaulPageCounter`

## Responsibility

Renders the current page label supplied by the owning controller.
...
Purpose: Initializes the page counter runtime and establishes its starting state and engine/UI integration.
...
- Mutates module/task state: `label`, `visible`.
...
Purpose: Handles the engine/UI `OnDraw` callback for the page counter runtime.
...
- Invokes engine/native operations: `native.Print`.
...
Purpose: Handles the engine/UI `OnUIMessage` callback for the page counter runtime.
...
- Mutates module/task state: `visible`, `label`.
