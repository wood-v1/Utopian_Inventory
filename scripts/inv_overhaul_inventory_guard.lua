maintask TEffect do
  ...
  function PublishBackpackRemovalHint(category: int, index: int) -> void
    ...
    native.Trace("inv_overhaul_quickslot removal hint ordinal=" + ordinal +
  ...
  function RemapPublishedLayout(category: int, count: int) -> void
    ...
    for cell = 0, c_iInventoryCapacity - 1 do
      local variableName: string = "inv_overhaul_inventory_cell_" + cell
      ...
      if oldOrder >= base && oldOrder < base + count then
        ...
        local mapName: string = "inv_overhaul_special_inventory_map_" + category + "_" + (oldOrder - base)
  ...
  function ClearQuickslotBinding(slot: int) -> void
    ...
    native.Trace("inv_overhaul_quickslot cleared exhausted binding slot=" + slot)
  end
  ...
  function ToggleQuickslotEquipment(category: int, index: int, itemID: int, selected: bool) -> void
    ...
    if category == c_iCWeapon then
      ...
      native.GetVariable("inv_overhaul_quickslot_active_weapon", activeWeapon)
      local shouldUnequip: bool = selected || activeWeapon == itemID
      native.Trace("inv_overhaul_quickslot weapon toggle item=" + itemID +
      ...
      if shouldUnequip then
        ...
        native.GetVariable("inv_overhaul_inventory_reorder_generation", generation)
        native.SetVariable("inv_overhaul_inventory_reorder_generation", generation + 1)
        native.Trace("inv_overhaul_quickslot unequipped weapon item=" + itemID)
      ...
      native.SetVariable("inv_overhaul_quickslot_active_weapon", itemID)
      local generation: int = 0
      native.GetVariable("inv_overhaul_inventory_reorder_generation", generation)
      native.SetVariable("inv_overhaul_inventory_reorder_generation", generation + 1)
      native.Trace("inv_overhaul_quickslot equipped weapon item=" + itemID)
      ShowQuickslotFeedback(itemID)
      return
    end
    ...
    if selected then
      ...
      player->SelectItem(index, false, category)
      native.Trace("inv_overhaul_quickslot unequipped category=" + category + " item=" + itemID)
      ShowQuickslotFeedback(itemID)
      return
    end
    ...
    player->SelectItem(index, true, category)
    native.Trace("inv_overhaul_quickslot equipped clothes item=" + itemID + " group=" + group)
    ShowQuickslotFeedback(itemID)
  end
  ...
  function UseQuickslotConsumable(slot: int, category: int, index: int, itemID: int) -> void
    ...
    if itemID == 42 || itemID == 55 || itemID == 72 then
      ...
      native.Trace("inv_overhaul_quickslot special use result=" + specialUsed +
    ...
    if effect == "" then
      native.Trace("inv_overhaul_quickslot item has no use effect item=" + itemID)
      ShowQuickslotMessage(c_iQuickslotUnusableTextID)
      return
    end
    local amount: int = amountBefore
    native.Trace("inv_overhaul_quickslot consume amount before=" + amountBefore +
      " category=" + category + " index=" + index + " item=" + itemID)
    amount = amount - 1
    if amount <= 0 then
    ...
    native.Trace("inv_overhaul_quickslot consume committed expected=" + amount +
      " actual=" + verifiedAmount + " category=" + category + " item=" + itemID)

    player->ApplyEffect(effect)
    native.Trace("inv_overhaul_quickslot applied effect after consume=" + effect +
  ...
  function ProcessQuickslotRequest() -> void
    local slot: int = 0
    native.GetVariable("inv_overhaul_quickslot_request", slot)
    if slot <= 0 then return end
    native.SetVariable("inv_overhaul_quickslot_request", 0)
    if slot > c_iQuickslotCount then
      return
    end
    native.SetVariable("inv_overhaul_quickslot_action_request", slot)
    native.Trace("inv_overhaul_quickslot guard opening inventory-context UI runtime slot=" + slot)
  ...
  function DropOverflowItem(index: int, itemID: int, category: int) -> bool
    ...
    native.Trace("inv_overhaul_inventory_guard dropped new item index=" + index + " item=" + itemID + " category=" + category)
    return true
  end
  ...
  function init() -> void
    ...
    native.GetVariable("inv_overhaul_quickslot_state_version", quickslotStateVersion)
