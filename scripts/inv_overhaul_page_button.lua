maintask InvOverhaulPageButton do
  ...
  function OnMouseEnter() -> void
    native.Trace("inv_overhaul_page_button diagnostics enter label=" + label + " visible=" + visible + " enabled=" + enabled)
  ...
  function OnMouseLeave() -> void
    native.Trace("inv_overhaul_page_button diagnostics leave label=" + label)
    highlighted = false
    UpdateBackground()
  ...
  function OnLButtonDown(x: int, y: int) -> void
    if visible && enabled then
      native.Trace("inv_overhaul_page_button down " + label)
      native.SendMessageToParent(0)
    end
  end

  function OnLButtonUp(x: int, y: int) -> void
    native.Trace("inv_overhaul_page_button up " + label)
  end
