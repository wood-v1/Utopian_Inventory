maintask UI_Cursor do
  ...
  function EnsureInitialized() -> void
    ...
    native.Trace("UTOPIAN_CURSOR_DIAG lazy initialization complete")
  end
  ...
  function LoadTooltip() -> void
    ...
    if !wndCur then
      ...
      tooltipObject = null
      tooltipTime = 0
      native.Trace("UTOPIAN_CURSOR_DIAG tooltip cleared no window")
      return
    end
    ...
    if tooltipObject then
      ...
      native.Trace("UTOPIAN_CURSOR_DIAG tooltip loaded type=" + tooltipType + " item=" + itemID)
    else
      native.Trace("UTOPIAN_CURSOR_DIAG tooltip loaded type=" + tooltipType + " no-item text='" + tooltipText + "'")
    end
    tooltipTime = 0
  end
  ...
  function OnCursorWndChange(newWindow: object, previousWindow: object) -> void
    EnsureInitialized()
    native.Trace("UTOPIAN_CURSOR_DIAG window change new=" + newWindow)
  ...
  function OnUpdate(delta: float) -> void
    ...
    if !updateSeen then
      updateSeen = true
      native.Trace("UTOPIAN_CURSOR_DIAG OnUpdate active")
    end
    native.GetVariable("utopian_inventory_drag_item", dragItemID)
    ...
    if tooltipType != c_iTooltipNone then
      ...
      if tooltipTime >= c_fOpaqueTime && !tooltipReadyLogged then
        ...
        native.Trace("UTOPIAN_CURSOR_DIAG tooltip delay reached type=" + tooltipType)
      end
    end
  end
  ...
  function OnDraw() -> void
    ...
    if !drawSeen then
      drawSeen = true
      native.Trace("UTOPIAN_CURSOR_DIAG OnDraw active")
    end
    native.GetVariable("utopian_inventory_drag_item", dragItemID)
    if dragItemID >= 0 then
      if dragItemID != loadedDragItemID then
    ...
    if alpha > 0 then
      if !tooltipDrawLogged then
        ...
        native.Trace("UTOPIAN_CURSOR_DIAG drawing tooltip type=" + tooltipType + " alpha=" + alpha)
      end
      local cursorX: int = 0
      local cursorY: int = 0
