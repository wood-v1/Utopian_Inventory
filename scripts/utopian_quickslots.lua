maintask UtopianQuickslotPlayerEffect do
  ...
  function UpdateTrackedWeapon() -> void
    local player: object = GetPlayer()
    local count: int
  ...
  function ScheduleHandsDrop() -> void
    if !m_bTrackedWeaponSelected || m_bPendingHandsDrop then
      native.Trace("utopian_drop_hands ignored no selected weapon")
      return
    end
    ...
    m_bTrackedWeaponSelected = false
    native.Trace("utopian_drop_hands scheduled item=" +
  ...
  function ProcessPendingHandsDrop(delta: float) -> void
    ...
    if index < 0 then
      ...
      native.Trace("utopian_drop_hands item vanished before drop item=" +
        m_iPendingHandsDropItemID)
      return
    end
    ...
    if selected && !holsterTransition then
      if m_fPendingHandsDropTimeout <= 0 then
        m_bPendingHandsDrop = false
        native.Trace("utopian_drop_hands cancelled weapon stayed selected item=" +
          m_iPendingHandsDropItemID)
      else
    ...
    native.Trace("utopian_drop_hands completed item=" +
      m_iPendingHandsDropItemID + " index=" + index)
  end

  function ProcessHandCombatRequest() -> void
    local requested: int = 0
    native.GetVariable("utopian_handcombat_request", requested)
    if requested <= 0 then return end
    native.SetVariable("utopian_handcombat_request", 0)
    native.Trace("utopian_drop_hands handcombat action requested")
    ScheduleHandsDrop()
  end
    ...
    for index = 0, count - 1 do
      ...
      if item then
        ...
        if itemID == targetItemID then
          ...
          native.Trace("utopian_quickslot diag " + tag +
  ...
  function ProcessPendingVerification(delta: float) -> void
    ...
    native.SetVariable("utopian_quickslot_diag_active", 0)
  end
  ...
  function ToggleEquipment(
    ...
    local player: object = GetPlayer()
    if category == c_iCWeapon then
      native.SetVariable("utopian_quickslot_weapon_item", itemID)
      native.SetVariable("utopian_quickslot_weapon_occurrence", occurrence)
      player->ApplyEffect("utopian_quickslot_weapon.bin")
      native.Trace("utopian_quickslot weapon action dispatched item=" +
  ...
  function ProcessPendingConsumption() -> void
    ...
    if index < 0 then
      native.Trace("utopian_quickslot pending item already consumed item=" +
        m_iPendingItemID)
    ...
    TraceInventoryState(
      ...
      m_iPendingCategory,
      m_iPendingItemID)
    native.Trace("utopian_quickslot pending consume committed slot=" +
  ...
  function UseConsumable(
    ...
    if effect == "" then
      native.Trace("utopian_quickslot unsupported consumable item=" + itemID)
    ...
    native.SetVariable("utopian_quickslot_diag_active", 1)
    ...
    native.Trace("utopian_quickslot player-effect applied=" + effect +
  ...
  function ActivateQuickslot(slot: int) -> void
    ...
    native.Trace("utopian_quickslot player-effect activate slot=" + slot +
  ...
  function init() -> void
    ...
    native.SetVariable("utopian_handcombat_request", 0)
    UpdateTrackedWeapon()
    native.Trace("UTOPIAN_QUICKSLOT_PLAYER_EFFECT_VERSION 2026.08.01-handcombat-config-3")
    while true do
      local delta: float
      native.sync(delta)
      ...
      local request: int = 0
      native.GetVariable("utopian_quickslot_request", request)
      if request > 0 then
